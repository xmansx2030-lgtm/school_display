...
المسح يعمل يوميًا داخل `email-notification-worker`. لتعجيله — أو لقراءة العدد قبل
كتابة أي شيء — شغّل **Close superseded trials** من تبويب Actions:

| الإدخال | المعنى |
|---|---|
| `mode` | `dry-run` (الافتراضي) يعرض العدد دون كتابة، `apply` ينفّذ |
| `school_id` | أرقام فقط، اتركه فارغًا لمسح كل المدارس |

الناتج يظهر في ملخّص التشغيل. الـ workflow مقصور على هذا الأمر وحده عمدًا: أي
workflow يقبل أمرًا حرًّا كإدخال يحوّل صلاحية الكتابة على المستودع إلى تنفيذ
شيفرة على خادم الإنتاج.
