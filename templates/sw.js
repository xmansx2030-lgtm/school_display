...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open('school-display-shell-v5')
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => !key.startsWith('school-display-') || !['school-display-shell-v5', 'school-display-runtime-v5'].includes(key)).map((key) => caches.delete(key))))
...
self.addEventListener('fetch', (event) => {
  ...
  if (isAsset) {
    event.respondWith(
      caches.open('school-display-runtime-v5').then(async (cache) => {
